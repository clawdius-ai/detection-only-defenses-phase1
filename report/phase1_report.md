# Phase 1 — Adaptive Jailbreak Attacks vs. Detector-Only Defenses
...
prompts in response to the target model's outputs and may consume feedback
...
cost-bounded subset of N=20 prompts is used for the initial reported run,
with the same pipeline supporting N=100 without code changes
...
configurable threshold (PromptGuard), when the classifier returns the
`unsafe` label (LlamaGuard), or when it matches a blocklist term (keyword).
...
- **Attacker**: `mistralai/mixtral-8x22b-instruct` via OpenRouter. The
  attacker prompt template follows the PAIR/TAP specification unchanged.
...
```
AdvBench behavior ──► Attacker (Mixtral-8x22B-Instruct)
                          │
                          ▼
                     candidate prompt
                          │
...
The experimental grid is fully driven by YAML configs and is reproducible
with:
...
The script iterates over every (attack × defense × budget) configuration
listed in `scripts/run_all.sh` and writes one JSONL trace per cell plus an
aggregated `results/summary.csv`.

## 6. Results

After execution, aggregated outcomes are written to `results/summary.csv`
and per-prompt outcomes to `results/per_prompt.csv`. A representative
...
JSONL trace is referenced in the matching `*_summary.txt` file.
...
- PromptGuard is a small (86M) input-classifier baseline; stronger
  detector baselines such as Llama-Guard-3-8B are included to satisfy the
...
- StrongREJECT and JBB-Behaviors are not yet evaluated; the dataset loader
  accepts arbitrary CSVs with `goal` and `target` columns, so additional
  datasets can be added by introducing new YAML configs.
